...
public class ClientHandler {
	...
	public static void playHordeSound(Vec3 vec3, ResourceLocation sound) {
		if (ClientConfigHandler.hordeSpawnSound.get()) {
			...
			BlockPos pos = BlockPos.m_274561_(player.getX() + (10 * vec3.x), player.getY(), player.getZ() + (10 * vec3.z));
			float pitch = 1 + ((level.random.nextInt(6) - 3) / 10);
			level.playSound(player, pos, SoundEvent.m_262824_(sound), SoundSource.HOSTILE, 0.5f, pitch);
		}
	}
