...
public class ClientHandler {
	...
	public void fogColour(EntityViewRenderEvent.FogColors event) {
		...
		if (optional.isPresent() && optional.orElseGet(null).isHordeNight(level)) {
			float d = level.getSkyDarken((float)event.getPartialTicks()) * 1.4f;
	...
	public static void playHordeSound(Vec3 vec3, ResourceLocation sound) {
		if (ClientConfigHandler.hordeSpawnSound.get()) {
			...
			level.playSound(player, pos, new SoundEvent(sound), SoundSource.HOSTILE, 0.5f, pitch);
		}
	}
	...
	public static void displayMessage(String text) {
		Minecraft mc = Minecraft.getInstance();
		Gui gui = mc.gui;
		MutableComponent message = new TranslatableComponent(text);
