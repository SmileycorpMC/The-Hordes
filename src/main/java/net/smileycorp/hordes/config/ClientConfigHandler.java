...
public class ClientConfigHandler {
	...
	private static int hordeMessageColour = -1;
	...
	public static int getHordeMessageColour() {
		if (hordeMessageColour == -1) {
			List<? extends Integer> rgb = configHordeMessageColour.get();
			if (rgb.size() >= 3) hordeMessageColour = (rgb.get(0) << 16) + (rgb.get(1) << 8) + rgb.get(2);
			else hordeMessageColour = 0;
		}
