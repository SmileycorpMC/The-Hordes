...
public class ClientInfectionEventHandler {
	...
	public void renderOverlay(RenderGameOverlayEvent event){
		...
		Gui.fill(event.getMatrixStack(), 0, 0, window.getWidth(), window.getHeight(), colour.getRGB());
	}
	...
	public void tooltip(RenderTooltipEvent.GatherComponents event) {
		...
		if (ClientConfigHandler.cureTooltip.get() && stack.m_204117_(HordesInfection.INFECTION_CURES_TAG))
			components.add(new TranslatableComponent("tooltip.hordes.cure"));
		if (ClientConfigHandler.immunityTooltip.get() && immunityItems.containsKey(item)) {
			components.add(new TranslatableComponent("potion.withDuration", new TranslatableComponent("effect.hordes.immunity"),
					MobEffectUtil.formatDuration(new MobEffectInstance(HordesInfection.IMMUNITY.get(),immunityItems.get(item) * 20), 1))
		...
		if (ClientConfigHandler.wearableProtectionTooltip.get() && wearableProtection.containsKey(item)) {
			...
			String str = value + "%";
			if (value > 0) str = "+" + str;
			components.add(new TranslatableComponent("tooltip.hordes.wearableProtection", str).withStyle(ChatFormatting.BLUE));
		}
