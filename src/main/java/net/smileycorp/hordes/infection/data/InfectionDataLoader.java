...
public class InfectionDataLoader extends JsonReloadListener {
    ...
    public void tryToInfect(LivingEntity entity, LivingEntity attacker, DamageSource source, float amount) {
        ...
        if ((entity instanceof PlayerEntity && InfectionConfig.infectPlayers.get()))
        ...
        if ((entity instanceof VillagerEntity && InfectionConfig.infectVillagers.get()))
    ...
    public boolean canBeInfected(Entity entity) {
        if (entity instanceof PlayerEntity) return InfectionConfig.infectPlayers.get();
        if (entity instanceof VillagerEntity && InfectionConfig.infectVillagers.get()) return true;
        if (!(entity instanceof MobEntity)) return false;
        return conversionTable.containsKey(entity.getType());
    }

    public boolean convertEntity(MobEntity entity) {
        InfectionConversionEntry entry = conversionTable.get(entity.getType());
    ...
    public boolean applyImmunity(LivingEntity entity, Item item) {
        if (immunityItems.containsKey(item)) {
            entity.addEffect(new EffectInstance(HordesInfection.IMMUNITY.get(), immunityItems.get(item) * 20));
            return true;
        }
        return false;
    }
    ...
    public float getModifiedInfectChance(LivingEntity entity, float chance) {
        for (EquipmentSlotType slot : EquipmentSlotType.values()) if (slot.getType() == EquipmentSlotType.Group.ARMOR)
    ...
    public void syncData(NetworkManager connection) {
