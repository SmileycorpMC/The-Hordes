...
public class Hordes {
	...
	public static void addPacks(AddPackFindersEvent event) {
		Path path = FMLPaths.CONFIGDIR.get().resolve(Constants.MODID);
		event.addRepositorySource(consumer -> consumer.accept(Pack.m_245429_(path.toString(), Component.literal("Hordes Config"), true,
				str -> new PathPackResources("hordes-config", true, path),
