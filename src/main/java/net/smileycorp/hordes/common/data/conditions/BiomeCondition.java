...
public class BiomeCondition implements Condition {
	...
	public boolean apply(Level level, LivingEntity entity, ServerPlayer player, Random rand) {
		...
		for (Either<TagKey<Biome>, ResourceLocation> either : biomes) if (either.map(biome::containsTag, biome::is)) return true;
		return false;
	}
	...
	private static Either<TagKey<Biome>, ResourceLocation> either(String string) {
		return string.contains("#") ? Either.left(TagKey.create(Registry.BIOME_REGISTRY, new ResourceLocation(string.replace("#", ""))))
