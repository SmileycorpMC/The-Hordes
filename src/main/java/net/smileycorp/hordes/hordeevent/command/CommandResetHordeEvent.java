...
public class CommandResetHordeEvent {
	...
	public static void register(LiteralArgumentBuilder<CommandSourceStack> command) {
		command.then(Commands.literal("reset").requires((commandSource) -> commandSource.hasPermission(1)).executes(CommandResetHordeEvent::execute)
				.then(Commands.argument("player", EntityArgument.players()).executes(ctx -> execute(ctx, EntityArgument.getPlayers(ctx, "player")))));
	}
