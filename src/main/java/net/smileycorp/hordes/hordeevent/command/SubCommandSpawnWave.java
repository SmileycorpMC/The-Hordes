...
public class SubCommandSpawnWave implements HordesCommand.SubCommand {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		if (args.length < 1 || args.length > 3) {
			throw new CommandException("commands." + Constants.MODID + ".SpawnHordeWave.usage");
		}
		try {
			if (args.length == 1) {
			...
			throw new CommandException("commands." + Constants.MODID + ".SpawnHordeWave.invalidValue", new TextComponentTranslation(args[0]));
		}
	}

	public static void execute(MinecraftServer server, ICommandSender sender, int count, Collection<EntityPlayerMP> players, ResourceLocation table) throws CommandException {
		server.addScheduledTask(() -> {
			for (EntityPlayerMP player : players) {
				...
				try {
					...
					HordesLogger.logError("Failed to run spawnWave command", e);
				}
			}
		});
		Entity entity = sender.getCommandSenderEntity();
		if (entity == null) return;
		entity.sendMessage(new TextComponentTranslation("commands." + Constants.MODID + ".SpawnHordeWave.success"));
	}
