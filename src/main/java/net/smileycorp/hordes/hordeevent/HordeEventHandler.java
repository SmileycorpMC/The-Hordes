...
public class HordeEventHandler {
	...
	public void playerTick(PlayerTickEvent event) {
		if (event.phase != Phase.END || !(event.player instanceof ServerPlayerEntity) || event.player instanceof FakePlayer) return;
		ServerPlayerEntity player = (ServerPlayerEntity) event.player;
		ServerWorld level = ServerLifecycleHooks.getCurrentServer().overworld();
		if (HordeEventConfig.pauseEventServer.get() && level.players().isEmpty()) return;
	...
	public void tryDespawn(LivingSpawnEvent.AllowDespawn event) {
		ServerPlayerEntity player = HordeSpawn.getHordePlayer(event.getEntity());
	...
	public void trySleep(PlayerSleepInBedEvent event) {
		if (HordeEventConfig.canSleepDuringHorde.get() || !(event.getEntity() instanceof ServerPlayerEntity)) return;
		...
		player.displayClientMessage(new TranslationTextComponent(Constants.hordeTrySleep), true);
	}
